...
module Driver
  ...
  use Lnd, only: modASS => SetServices
  use ATM, only: modBSS => SetServices
  ...
  character(len=10), parameter :: standardNames(6) =(/ &
  "DeltaT_A  ", &
  "e         ", &
  "f         ", &
  "DeltaT_L  ", &
  "alpha     ", &
  "beta      "/)
  character(len=10), parameter :: units(6) =(/ &
  "K         ", &
  "1         ", &
  "1         ", &
  "K         ", &
  ...
  subroutine SetModelServices(driver, rc)
    ...
    ! SetServices for Lnd and ATM with petList on second half of PETs
    allocate(petList(petCount/2))
    do i=1, petCount/2
      petList(i) = petCount/2 + i-1 ! PET labeling goes from 0 to petCount-1
    enddo
    call NUOPC_DriverAddComp(driver, "LND", modASS, petList=petList, &
      comp=child, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      ...
      file=__FILE__)) &
      return  ! bail out
    call NUOPC_DriverAddComp(driver, "ATM", modBSS, petList=petList, &
      comp=child, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
    ...
    do i=1, 6
      call NUOPC_FieldDictionaryAddEntry(standardName=standardNames(i), &
                 canonicalUnits=units(i),rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return  ! bail out
  ...
  subroutine SetRunSequence(driver, rc)
    ...
    runSeqFF = NUOPC_FreeFormatCreate(stringList=(/ &
      " @*                             ",    &
      "   Mediator sfc_boundary_layer  ",    &
      "   ATM      atmos_down          ",    &
      "   Mediator flux_down_from_atmos",    &
      "   Mediator -> LND              ",    &
      "   LND                          ",    &
      "   LND -> Mediator              ",    &
      "   Mediator flux_up_to_atmos    ",    &
      "   Mediator -> ATM              ",    &
      "   ATM      atmos_up            ",    &
      "   ATM -> Mediator              ",    &
      " @                              " /), &
