...
module Lnd
  ...
  subroutine InitializeAdvertise(gcomp, importState, exportState, clock, rc)
    ...
    ! importable field: alpha
    call NUOPC_Advertise(importState, &
      StandardName="alpha", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    
    ! importable field: beta
    call NUOPC_Advertise(importState, &
      StandardName="beta", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out

    ! exportable field: DeltaT_L
    call NUOPC_Advertise(exportState, &
      StandardName="DeltaT_L", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
  ...
  subroutine InitializeRealize(gcomp, importState, exportState, clock, rc)
...
#endif
#if 1
    !gridIn=ESMF_GridCreate1PeriDimUfrm(maxIndex=(/20,20/), &
    ...
    ! importable field: alpha
    field = ESMF_FieldCreate(name="alpha", &
    ...
    ! importable field: beta
    field = ESMF_FieldCreate(name="beta", &
      grid=gridIn, typekind=ESMF_TYPEKIND_R8, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
    ...
    ! exportable field: DeltaT_L
    field = ESMF_FieldCreate(name="DeltaT_L", &
...
#ifdef WRITE_IMPEXP
    call NUOPC_Write(importState, filenamePrefix='lnd_imp_', &
...
#ifdef WRITE_IMPEXP
    call NUOPC_Write(exportState, filenamePrefix='lnd_exp_', &
